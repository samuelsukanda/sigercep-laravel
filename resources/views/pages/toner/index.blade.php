...
@section('content')
    <div class="w-full px-6 py-6 mx-auto">
        <div class="flex justify-between items-center mb-4">
            <h6 class="text-xl font-bold text-slate-700 dark:text-white">Daftar Toner</h6>

            @canAccess('toner', 'create')
            <x-button.link href="{{ route('toner.create') }}">
                Tambah Data
            </x-button.link>
            @endcanAccess
        </div>
        ...
        <div class="flex justify-end items-center mb-4 flex-wrap gap-2">
            {{-- Filter Tanggal --}}
            <form method="GET" action="{{ route('toner.index') }}" class="flex items-center gap-4 mb-4">
                <div>
                ...
                <tbody class="text-s text-slate-500 bg-slate-100 dark:text-white">
                    @foreach ($toner as $item)
                        <tr>
                            ...
                            <td class="px-6 py-4">{{ $item->toner }}</td>
                            ...
                            <td class="px-6 py-4 space-x-2 text-center">
                                ...
                                <x-button.action href="{{ route('toner.destroy', $item->id) }}" icon="trash"
