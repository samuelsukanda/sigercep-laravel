...
@section('title', 'SIGERCEP - Detail Toner')
            ...
            <div class="w-full max-w-full px-3 mx-auto mt-0">
                <div class="relative flex flex-col bg-white shadow-soft-xl rounded-2xl">
                    <div class="p-6 pb-0 mb-0 bg-white rounded-t-2xl">
                        <h6 class="mb-0 font-bold text-lg">Detail Toner</h6>
                    </div>
                    <div class="flex-auto p-6">
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            <div>
                                ...
                                <p class="text-slate-600">{{ $toner->nama }}</p>
                            </div>

                            {{-- Unit --}}
                            <div>
                                <label class="block mb-1 text-sm font-semibold text-slate-700">Unit</label>
                                <p class="text-slate-600">{{ $toner->unit }}</p>
                            </div>

                            {{-- Jenis Toner --}}
                            <div>
                                <label class="block mb-1 text-sm font-semibold text-slate-700">Jenis Toner</label>
                                <p class="text-slate-600">{{ $toner->toner }}</p>
                            </div>

                            {{-- Jumlah --}}
                            <div>
                                <label class="block mb-1 text-sm font-semibold text-slate-700">Jumlah</label>
                                <p class="text-slate-600">{{ $toner->jumlah }}</p>
                            </div>
                            ...
                            <div>
                                ...
                                <p class="text-slate-600">
                                    {{ \Carbon\Carbon::parse($toner->tanggal)->translatedFormat('d F Y') }}
                                </p>
                            </div>
                            ...
                            @if ($toner->tanda_tangan)
                                <div class="mb-4">
                                    ...
                                    <img src="{{ asset($toner->tanda_tangan) }}" alt="Tanda Tangan Pelapor"
