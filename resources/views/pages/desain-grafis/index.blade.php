...
@section('content')
    <div class="w-full px-6 py-6 mx-auto">
        <div class="flex justify-between items-center mb-4">
            <h6 class="text-xl font-bold text-slate-700 dark:text-white">Daftar Desain Grafis</h6>

            @canAccess('desain_grafis', 'create')
            <x-button.link href="{{ route('desain-grafis.create') }}">
                Tambah Data
            </x-button.link>
            @endcanAccess
        </div>
        ...
        <div class="flex justify-end items-center mb-4 flex-wrap gap-2">
            {{-- Filter Tanggal --}}
            <form method="GET" action="{{ route('desain-grafis.index') }}" class="flex items-center gap-4 mb-4">
                <div>
