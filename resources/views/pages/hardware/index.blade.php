...
@section('content')
    <div class="w-full px-6 py-6 mx-auto">
        <div class="flex justify-between items-center mb-4">
            <h6 class="text-xl font-bold text-slate-700 dark:text-white">Daftar Ceklis Hardware</h6>

            @canAccess('hardware', 'create')
            <x-button.link href="{{ route('hardware.create') }}">
                Tambah Data
            </x-button.link>
            @endcanAccess
        </div>
        ...
        <div class="flex justify-end items-center mb-4 flex-wrap gap-2">
            {{-- Filter Tanggal --}}
            <form method="GET" action="{{ route('hardware.index') }}" class="flex items-center gap-4 mb-4">
                <div>
                    ...
                    @foreach ($hardware as $item)
                        <tr>
                            ...
                            <td class="px-6 py-4 space-x-2 text-center">
                                ...
                                <x-button.action href="{{ route('hardware.show', $item->id) }}" icon="eye"
                                    color="emerald" title="Lihat Data" />
                                @endcanAccess

                                @canAccess('hardware', 'delete')
                                <x-button.action href="{{ route('hardware.destroy', $item->id) }}" icon="trash"
