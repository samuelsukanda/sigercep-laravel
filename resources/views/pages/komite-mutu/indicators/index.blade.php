                    ...
                    <div class="flex-auto p-4">
                        <div class="flex flex-row -mx-3">
                            <div class="flex-none w-2/3 max-w-full px-3">
                                <div>
                                    <p class="mb-0 font-sans text-xs font-semibold leading-normal uppercase text-slate-400 dark:text-white dark:opacity-60">Total Indikator ({{ $jenis }})</p>
                ...
                <div class="relative flex flex-col min-w-0 break-words bg-white shadow-xl dark:bg-slate-850 rounded-2xl bg-clip-border">
                    <div class="flex-auto p-4">
                        <div class="flex flex-row -mx-3">
                            <div class="flex-none w-2/3 max-w-full px-3">
                                <div>
                                    <p class="mb-0 font-sans text-xs font-semibold leading-normal uppercase text-slate-400 dark:text-white dark:opacity-60">Tingkat Capaian Target</p>
                ...
                <div class="relative flex flex-col min-w-0 break-words bg-white shadow-xl dark:bg-slate-850 rounded-2xl bg-clip-border">
                    <div class="flex-auto p-4">
                        <div class="flex flex-row -mx-3">
                            <div class="flex-none w-2/3 max-w-full px-3">
                                <div>
                                    <p class="mb-0 font-sans text-xs font-semibold leading-normal uppercase text-slate-400 dark:text-white dark:opacity-60">Tahun Monitoring</p>
                                    <h4 class="mb-0 font-bold text-2xl dark:text-white">{{ $tahun }}</h4>
                                </div>
                            </div>
                            <div class="px-3 text-right basis-1/3">
                                <div class="inline-flex items-center justify-center w-12 h-12 rounded-full bg-gradient-to-tl from-orange-500 to-amber-400 shadow-soft-2xl">
                ...
                <div class="relative flex flex-col min-w-0 break-words bg-white shadow-xl dark:bg-slate-850 rounded-2xl bg-clip-border">
                    <div class="flex-auto p-4">
                        <div class="flex flex-row -mx-3">
                            <div class="flex-none w-2/3 max-w-full px-3">
                                <div>
                                    <p class="mb-0 font-sans text-xs font-semibold leading-normal uppercase text-slate-400 dark:text-white dark:opacity-60">Kategori</p>
                                    <h4 class="mb-0 font-bold text-2xl dark:text-white">{{ $jenis }}</h4>
                                </div>
                            </div>
                            <div class="px-3 text-right basis-1/3">
                                <div class="inline-flex items-center justify-center w-12 h-12 rounded-full bg-gradient-to-tl from-red-500 to-pink-500 shadow-soft-2xl">
        ...
        <div class="bg-white dark:bg-slate-850 rounded-2xl shadow-xl border border-gray-200 dark:border-slate-800 overflow-hidden mb-8">
            ...
            <div class="p-6 border-b border-gray-100 dark:border-slate-800 bg-gray-50/50 dark:bg-slate-900/50">
                <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                    ...
                    {{-- Year Selector --}}
                    <form method="GET" action="{{ route('indicators.index') }}" id="filterForm" class="flex items-center gap-2">
                        <label for="tahunSelect" class="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase mr-2">Tahun:</label>
                ...
                <table class="w-full text-left border-collapse border border-gray-300 dark:border-slate-700">
                    <thead>
                        <tr class="text-slate-400 dark:text-slate-300 text-xs font-bold uppercase">
                            ...
                            <th class="py-3 px-4 min-w-[280px] border border-gray-300 dark:border-slate-700">Indikator Mutu</th>
                            <th class="py-3 px-4 min-w-[120px] border border-gray-300 dark:border-slate-700">PJ / Unit</th>
                            <th class="py-3 px-3 text-center min-w-[80px] border border-gray-300 dark:border-slate-700">Target</th>
                            @for ($m = 1; $m <= 12; $m++)
                                <th class="py-3 px-2 text-center w-14 border border-gray-300 dark:border-slate-700">
                                    {{ substr(date('F', mktime(0, 0, 0, $m, 1)), 0, 3) }}
                                </th>
                            @endfor
                            <th class="py-3 px-3 text-center min-w-[80px] bg-slate-50 dark:bg-slate-900/30 border border-gray-300 dark:border-slate-700">Rata-Rata</th>
                        </tr>
                    </thead>
                    <tbody class="divide-y divide-gray-100 dark:divide-slate-800 text-sm text-slate-600 dark:text-slate-300">
                        @forelse ($indicators as $indicator)
                            ...
                            <tr class="hover:bg-gray-50/50 dark:hover:bg-slate-800/30 transition-colors">
                                {{-- No Urut --}}
                                <td class="py-4 px-3 text-center font-bold text-xs text-slate-400 border border-gray-300 dark:border-slate-700">
                                    {{ $indicator->no_urut }}
                                </td>
                                ...
                                <td class="py-4 px-4 text-xs font-medium text-slate-500 dark:text-slate-400 border border-gray-300 dark:border-slate-700">
                                    <span class="block font-bold text-slate-700 dark:text-slate-300">{{ $indicator->pj }}</span>
                                    @if ($indicator->unit_terkait)
                                        <span class="block text-[10px] text-slate-400 mt-0.5">{{ $indicator->unit_terkait }}</span>
                                    @endif
                                </td>
                                
                                {{-- Target --}}
                                <td class="py-4 px-3 text-center font-bold text-slate-700 dark:text-slate-300 border border-gray-300 dark:border-slate-700">
                                    @if ($target !== null)
                                ...
                                @for ($m = 1; $m <= 12; $m++)
                                    @php
                                        ...
                                        if ($numerator !== null && $denominator !== null) {
                                            $tooltipText = "Numerator: " . (float)$numerator . " | Denominator: " . (float)$denominator;
                                        }
                                    @endphp
                                ...
                                @endphp
                                <td class="py-4 px-3 text-center text-xs font-bold bg-slate-50/50 dark:bg-slate-900/20 {{ $avgClass }} border border-gray-300 dark:border-slate-700">
                                    @if ($average !== null)
                                        {{ $average }}%
                                    @else
                                        -
                        ...
                        @empty
                            <tr>
                                <td colspan="17" class="py-12 text-center text-slate-500 border border-gray-300 dark:border-slate-700">
                                    <div class="flex flex-col items-center justify-center gap-3">
                                        <i class="fas fa-folder-open text-4xl text-slate-300"></i>
                                        <span class="font-bold text-slate-400">Tidak ada data indikator ditemukan untuk kategori ini.</span>
                                    </div>
                                </td>
                            </tr>
                        @endforelse
