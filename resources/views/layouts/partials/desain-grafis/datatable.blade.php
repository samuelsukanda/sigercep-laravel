<div class="relative overflow-x-auto shadow-md rounded-lg px-2 bg-white dark:text-white">
    <table id="desainGrafisTable" class="datatable-custom min-w-full divide-y divide-gray-200 dark:divide-white-200 dark:text-white">
        <thead class="text-xs text-slate-500 uppercase bg-white dark:text-white">
            <tr>
                <th class="px-6 py-3">Nama</th>
                <th class="px-6 py-3">Unit</th>
                <th class="px-6 py-3">Keperluan</th>
                <th class="px-6 py-3">Jenis Desain</th>
        ...
        <tbody class="text-s text-slate-500 bg-white">
            @forelse ($desain as $item)
                <tr class="hover:bg-gray-50 transition-colors">
                    ...
                    <td class="px-6 py-4">
                        <span
                            class="px-2 py-1 text-xs font-semibold rounded-full {{ $item->desain == 'Video' ? 'bg-red-100 text-red-800' : 'bg-blue-100 text-blue-800' }}">
                            {{ $item->desain }}
                        </span>
                    </td>
                    ...
                    <td class="px-6 py-4 space-x-2 text-center">
                        @canAccess('desain_grafis', 'update')
                        ...
                        @canAccess('desain_grafis', 'delete')
                ...
                <tr>
                    <td colspan="7" class="px-6 py-8 text-center text-gray-500">
                        <div class="flex flex-col items-center gap-2">
                            ...
                            <p>Tidak ada data desain grafis ditemukan</p>
                        </div>
                    </td>
                </tr>
            @endforelse
